...
3. Paste the output to the end of init_db.py and make sure the indentation and the format is the same. In there, do not replace the current values because that will be the entire database. Only delete if you want to eliminate devices from the website.
...
6. Paste the copied outputs from step 2 into the file generate_qr.py and run it to generate the respective qr codes, which will appear in the folder static\qrcodes. The script will generate a qr code for all of the devices in the file, so delete the ones you already generated or don't need.
    6.1. In addition to the QR codes this website now includes asset certificates so make sure the name follows the convention: "cert-device.name" 
